...
* waran from #delphigl
  For his great support on asm related questions.
*******************************************************************************)
unit Thorium;
...
{$ifdef ENDIAN_BIG}
{$error BigEndian plattform *will* cause problems with Thorium NativeCall. So fasten your seatbelts, get popcorn and watch the catastrophy.}
...
type
  ...
  {$PACKENUM 2}
  TThoriumNativeCallInstructionCode = (ccSkipRegister, ccIncStrRef{$ifndef CPU64}, ccVARev{$endif}, ccPtrDeref, ccDecStrRef, ccData, ccVA, ccInt, ccInt64, ccSingle, ccDouble, ccExtended, ccIntRef, ccInt64Ref, ccSingleRef, ccDoubleRef, ccExtendedRef, ccCall, ccCallRetInt, ccCallRetInt64, ccCallRetSingle, ccCallRetDouble, ccCallRetExtended, ccCallRetString, ccDecStack, ccRevSingle, ccRevExtended, ccExit);
  ...
  {$M+}
  TThoriumPersistent = class (TObject, IUnknown, IThoriumPersistent)
  public
    {$ifdef DebugToConsole}
  ...
  TThoriumHostFunctionBase = class (TThoriumHashableObject)
  ...
  TThoriumHostMethodBase = class (TThoriumHostFunctionBase)
  public
    constructor Create; override;
  protected
    FExtendedType: TThoriumHostObjectType;
  protected
    procedure CallFromVirtualMachine(AVirtualMachine: TThoriumVirtualMachine=nil); override; overload;
    procedure CallFromVirtualMachine(OfObject: TObject; AVirtualMachine: TThoriumVirtualMachine=nil); virtual; abstract; overload;
  public
  end;
  ...
  protected
    ...
    procedure AssignValue(const ASource: TThoriumValue; var ADest: TThoriumValue); virtual; abstract;
  ...
  TThoriumRTTIObjectType = class (TThoriumHostObjectType)
    constructor Create(ALibrary: TThoriumLibrary); override; overload;
    constructor Create(ALibrary: TThoriumLibrary;
      ABaseClass: TThoriumPersistentClass; AbstractClass: Boolean = False); overload;
    constructor Create(ALibrary: TThoriumLibrary; ABaseClass: TClass;
      ...
      AbstractClass: Boolean = False); overload;
    destructor Destroy; override;
  private
    FBaseClass: TClass;
  ...
  TThoriumScanner = class (TObject)
    constructor Create(AInputString: String); overload;
  ...
  public
    ...
    function AppendCode(AInstruction: TThoriumInstruction): Integer; overload;
    function AppendCode(Code: TThoriumInstructionArray): Integer; overload;
  ...
  TThoriumModule = class (TThoriumHashableObject)
    constructor Create(AThorium: TThorium); virtual; overload;
    constructor Create(AThorium: TThorium; AName: String); overload;
    destructor Destroy; override;
  private
    FCompiled: Boolean;
    FCompress: Boolean;
  ...
  private
    ...
    function AddHostFunctionUsage(const AFunc: TThoriumHostFunctionBase): Integer;
    function AddHostFunctionUsageToRelocate(const AFunc: TThoriumHostFunctionBase; const AOffset: ptruint): Integer;
  ...
  protected
    procedure AddDependency(const ALibName: String); overload;
    procedure AddDependency(const ALib: TThoriumLibrary); overload;
    procedure ClearAll;
    ...
    function RegisterRTTIType(const AClass: TThoriumPersistentClass;
      AbstractClass: Boolean = False): TThoriumRTTIObjectType; overload;
  ...
  public
    ...
    function Push: PThoriumStackEntry; inline; overload;
    procedure Push(AEntry: PThoriumStackEntry); inline; overload;
  ...
  public
    ...
    procedure Execute(StartModuleIndex: Integer; StartInstruction: Integer; CreateDefaultStackFrame: Boolean = False);
  end;
...
const
  ...
  THORIUM_INT_FLT_OPS = [toAdd, toSubtract, toMultiply, toDivide, toAssign,
    toCompare];
  THORIUM_INT_STR_OPS = [];
...
//function ThoriumOperatorKindToStr(AKind: Word): String;
...
//function VariantToVarRec(const AVariant: Variant): TVarRec;
...
function moves(ATRI: Word): TThoriumInstruction;
begin
  FillByte(Result, SizeOf(TThoriumInstruction), 0);
...
function call(AEntryPoint: TThoriumInstructionAddress; AHRI: Word; ARetVal: Word; AParameters: Cardinal): TThoriumInstruction;
begin
  FillByte(Result, SizeOf(TThoriumInstruction), 0);
...
{ The following functions were taken from astrings.inc and have been modified
  by the author of Thorium to fit the special needs of the direct call feature. }
...
const
  ...
  InstrMap: array [TThoriumNativeCallInstructionCode] of Pointer = (
    ...
    @iCallRetSingle, @iCallRetDouble, @iCallRetExtended, @iCallRetString,
...
var
  EAX, ECX, EDX, SEBX, SECX, SEDX, SESI, SESP, SEDI: DWord;
begin
  // eax : buffer
...
begin
  if VarType^.HostType = htExt then
  begin
...
var
  MemAmount: Integer;
begin
  MemAmount := TypeClass.GetNeededMemoryAmount;
  Result._Type := vtExtendedType;
  Result.Extended.TypeClass := TypeClass;
  if MemAmount > 0 then
  begin
    GetMem(Result.Extended.Value, MemAmount);
    Result.Extended.Size := MemAmount;
  end
  else
    Result.Extended.Size := 0;
end;
...
begin
  ...
  case Result._Type of
    ...
    begin
      ...
      Result.Extended.Value := GetMem(Result.Extended.Size);
    end;
  end;
end;
...
begin
  ...
  case AValue._Type of
    ...
    begin
      ...
      if AValue.Extended.Size > 0 then
        FreeMem(AValue.Extended.Value);
    end;
  end;
end;
...
begin
  WriteLn(ClassName, ': Free called at ', FReferences, ' references');
end;
{$endif}
...
begin
  WriteLn(ClassName, ': Destructing at ', FReferences, ' references');
  inherited Destroy;
end;
{$endif}

procedure TThoriumPersistent.FreeReference;
begin
  {$ifdef DebugToConsole}
  WriteLn(ClassName, ': FreeReference (going to be at ', FReferences-1, ' reference(s))');
  {$endif}
  if InterLockedDecrement(FReferences) = 0 then
    Free;
end;

function TThoriumPersistent._AddRef: LongInt; stdcall;
begin
  Result := InterLockedIncrement(FReferences);
end;

function TThoriumPersistent._Release: LongInt; stdcall;
begin
...
function TThoriumPersistent.QueryInterface(const IID: TGuid; out Obj): LongInt;
  stdcall;
begin
...
function TThoriumPersistent.GetReference: TObject;
begin
  {$ifdef DebugToConsole}
  WriteLn(ClassName, ': GetReference (currently ', FReferences, ' references)');
  {$endif}
  InterLockedIncrement(FReferences);
  {$ifdef DebugToConsole}
  WriteLn(ClassName, ': GetReference (now at ', FReferences, ' references)');
  {$endif}
  Result := Self;
end;

function TThoriumPersistent.GetReferenceCount: Integer;
begin
  Result := FReferences;
end;
...
begin
  ...
  begin
    ...
    begin
      Entry := PThoriumTableEntry(ptruint(FIdentifiers) + I * SizeOf(TThoriumTableEntry));
...
begin
  ...
  begin
    ...
    if not ThoriumCompareType(ThoriumExtractTypeSpec(AParameters[I]), ParamSpec) then
...
var
  ...
  LocalList: PInteger;
begin
  //for I := FCount-1 downto FromIdx do
  //  TThoriumInstructionJMP(Instructions.Instruction[FList[I]]^).NewAddress += Offset;
  LocalList := FList;
  for I := 0 to FCount - 1 do
  begin
...
function TThoriumRTTIObjectType.GetNeededMemoryAmount: TThoriumSizeInt;
begin
  // Return the size of a pointer as needed memory amount.
  Result := 0;//SizeOf(ptruint);
end;
...
begin
  ...
  if FCanUsePersistent then
    TThoriumPersistent(Result.Extended.Value) := TThoriumPersistent(TThoriumPersistent(AValue.Value).GetReference)
  else
  begin
...
begin
  ...
  begin
    if TThoriumPersistent(AValue.Value) <> nil then
      TThoriumPersistent(AValue.Value).FreeReference;
  end
  else
  begin
  ...
  begin
    ...
    case Info^.PropType^.Kind of
      ...
      begin
        ...
        if (ObjClass = TThoriumPersistent) or ObjClass.InheritsFrom(TThoriumPersistent) then
        begin
          ExtType := FLibrary.DeepFindRTTIType(ObjClass.ClassName); //FRegistry.FindRTTIType(TThoriumPersistentClass(ObjClass));
          if ExtType <> nil then
...
begin
  ...
  case Info^.PropType^.Kind of
    ...
    begin
      ...
      Result.Extended.Value := TThoriumPersistent(GetObjectProp(TObject(AInstance.Extended.Value), Info)).GetReference;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    ReturnType.HostType := AReturnType;
...
{ TThroiumExternalFunctionBase }

constructor TThoriumHostFunctionBase.Create;
begin
  inherited Create;
  FName := '';
...
destructor TThoriumHostFunctionBase.Destroy;
begin
  FParameters.Free;
  inherited Destroy;
end;

procedure TThoriumHostFunctionBase.CalcHash;
var
  Signature: String;
  SigLength: Integer;
  Offset: Integer;
...
begin
  ...
  Stack.Pop(Parameters.Count + FVAOffset);
end;
...
begin
  ...
  Stack.Pop(FParameters.Count + FVAOffset);
end;
...
procedure TThoriumHostMethodBase.CallFromVirtualMachine(AVirtualMachine: TThoriumVirtualMachine);
begin
  raise EThoriumRuntimeException.Create('External method called without self pointer.');
end;
...
function TThoriumModule.AddHostFunctionUsage(const AFunc: TThoriumHostFunctionBase): Integer;
begin
...
function TThoriumModule.AddHostFunctionUsageToRelocate(const AFunc: TThoriumHostFunctionBase;
  const AOffset: ptruint): Integer;
var
  Info: PThoriumRelocation;
begin
...
procedure TThoriumModule.FindRelocationTargets;
var
...
procedure TThoriumModule.OptimizeCode;
...
var
  ...
  const
    InstructionSet1 = [tiINTB, tiSTR, tiCOPYR_S, tiCOPYR_ST, tiCOPYR_FS,
      tiCOPYR, tiMOVER_S, tiMOVER_ST, tiMOVES, tiCLR, tiCMPI, tiCMPIF,
  ...
  begin
    ...
    Copy := Instruction;
    Move := @Instruction[1];
    if (Copy^.Instruction <> tiMOVES) or (Move^.Instruction <> tiMOVER_ST) then
      Exit;
    if (TThoriumInstructionMOVES(Copy^).TRI = TThoriumInstructionMOVER_ST(Move^).SRI) then
    begin
...
begin
  ...
  begin
    ...
    InstructionHash := MD5Buffer(FInstructions.Instruction[0]^, SizeOf(TThoriumInstruction)*FInstructions.Count);
  end;
...
begin
  ...
  for I := 0 to Header.HostFuncDependencyCount - 1 do
  begin
    ExternalFunc := TThoriumHostFunctionBase(FHostFuncUsage[I]);
    Stream.WriteAnsiString(ExternalFunc.Name);
    if ExternalFunc is TThoriumHostMethodBase then
    begin
      Stream.Write(1, SizeOf(Byte));
      J := FHostTypeUsage.IndexOf(TThoriumHostMethodBase(ExternalFunc).FExtendedType);
...
function TThoriumModule.CompileFromStream(SourceStream: TStream): Boolean;
...
var
  ...
  procedure CompilerError(Msg: String); inline;
  // Sets the local compiler error message and sets the result of CompileCode to
  // false
  begin
    FLastCompilerError := '('+IntToStr(Scanner.CurrentLine)+' | '+IntTostr(Scanner.FCurrentX)+'): '+Msg;
    // WriteLn(FLastCompilerError);
    CompileFromStream := False;
  end;
  ...
  // Checks if CompileCode has an error flag and if so, return true.
  begin
    Result := not CompileFromStream;
  end;
  ...
  begin
    ...
    begin
      TypeSpec.Extended := FindHostObjectType(Ident); //FThorium.FExtendedTypeRegistry.GetTypeByName(Ident);
  ...
  begin
    ...
    begin
      ...
      CurrExternalFunc := FindHostFunction(Ident); // FThorium.FExternalFunctionRegistry.FindFunctionDirect(Ident);
      ...
      CurrProp := FindLibraryProperty(Ident);
      ...
      // be a constant then.
      CurrConst := FindLibraryConstant(Ident);
  ...
  var
    ...
    function Expression(TargetRegister: TThoriumRegisterID; NeedStatic: Boolean = False; IsStatic: PBoolean = nil; StaticValue: PThoriumValue = nil): TThoriumType; forward;
    ...
    var
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              // ThoriumVarTypeToTypeSpec(Func.Parameters.Types[I] and (not vararray), ParamType);
            ...
            repeat
              ...
              ExprType := Expression(ParamRegID);
              if HasError then
                Exit;
              ...
              begin
                ...
                if not ThoriumCompareType(ParamType, OldExprType) then
                  GenCode(_cast(ParamRegID, ParamRegID, ParamType, ExprType));
          ...
          begin
            ...
            if HasError then
              Exit;
            // Get the parameter type spec
            ThoriumExternalVarTypeToTypeSpec(Func.Parameters.CompleteTypes[I], ParamType);
            // Check if the types are compatible.
            OldExprType := ExprType;
      ...
      var
        I: Integer;
        Func: TThoriumFunction;
        ExprType, ParamType: TThoriumType;
        ParamRegID: TThoriumRegisterID;
      begin
        // Get the function
        Func := CurrType.Func;
      ...
      try
        ...
        begin
          case CurrEntry._Type of
            etRegisterVariable:
            begin
              GenCodeEx(Ident.GetCode, copyr(CurrEntry.Offset, TargetRegister));
            ...
            begin
              ...
              begin
                ...
                // operations.
                GenCodeEx(Ident.GetCode, copyfs(CurrModule, CurrEntry.Offset, TargetRegister));
              ...
              begin
                ...
                GenCodeEx(Ident.GetCode, copys(CurrEntry.Scope, CurrEntry.Offset, TargetRegister));
        ...
        begin
          ...
          begin
            ...
            case BufInstruction^.Instruction of
              tiCOPYS: TThoriumInstructionCOPYS(BufInstruction^).TRI := RegID;
              tiCOPYR_S: TThoriumInstructionCOPYR_S(BufInstruction^).SRI := RegID;
              tiCOPYR_FS: TThoriumInstructionCOPYR_FS(BufInstruction^).SRI := RegID;
              tiCOPYFS: TThoriumInstructionCOPYFS(BufInstruction^).TRI := RegID;
    ...
    begin
      ...
      if not ThoriumCompareType(TypeSpec, TypeIdent.FinalType) then
      begin
    ...
    begin
      ...
      begin
        Proceed;
        // ... okay, there is one, now check if it's a perhaps valid value
        if not ExpectSymbol([tsIntegerValue, tsFloatValue, tsStringValue, tsNil]) then
          Exit;
        case TypeIdent.FinalType.ValueType of
          vtBuiltIn: // It's a built in value
          begin
            case TypeIdent.FinalType.BuiltInType of
              btInteger: // and an integer
              begin
                if CurrentSym <> tsIntegerValue then
                begin
                  CompilerError('Illegal initial value for int.');
                  Exit;
                end;

                if RegisterVariable = THORIUM_REGISTER_INVALID then
                  GenCode(int_s(StrToInt64(CurrentStr)))
                else
                  GenCode(int(StrToInt64(CurrentStr), RegisterVariable));
              end;
              btFloat: // and a float
              begin
                if not (CurrentSym in [tsIntegerValue, tsFloatValue]) then
                begin
                  CompilerError('Illegal initial value for float.');
                  Exit;
                end;

                if RegisterVariable = THORIUM_REGISTER_INVALID then
                  GenCode(flt_s(StrToFloat(CurrentStr, THORIUM_NUMBER_FORMAT)))
                else
                  GenCode(flt(StrToFloat(CurrentStr, THORIUM_NUMBER_FORMAT), RegisterVariable));
              end;
              btString: // and a string
              begin
                if CurrentSym <> tsStringValue then
                begin
                  CompilerError('Illegal initial value for string.');
                  Exit;
                end;
                if (CurrentStr <> '') then
                begin
                  if RegisterVariable = THORIUM_REGISTER_INVALID then
                    GenCode(strl_s(AddLibraryString(CurrentStr)))
                  else
                    GenCode(strl(AddLibraryString(CurrentStr), RegisterVariable));
                end
                else
                begin
                  if RegisterVariable = THORIUM_REGISTER_INVALID then
          ...
          vtExtendedType: // It's an extended type
          begin
            // ... so we can only accept a nil value
            if CurrentSym <> tsNil then
            begin
              CompilerError('Illegal initial value for '+ThoriumTypeName(TypeIdent.FinalType)+'.');
              Exit;
            end;

            // Write the instruction which pushes a new value of the specified
            // type to the stack.
            AddHostTypeUsage(TypeIdent.FinalType.Extended);
            GenCode(ext_s(TypeIdent.FinalType.Extended));
          end;
        end;
        Proceed;
      end
    ...
    function SimpleExpression(TargetRegister: TThoriumRegisterID; NeedStatic: Boolean = False; IsStatic: PBoolean = nil; StaticValue: PThoriumValue = nil): TThoriumType;
    // This pareses a simple expresion. A simple expression consists of
    // Terms which can be added.

      function Term(TargetRegister: TThoriumRegisterID; out IsStatic: Boolean; out StaticValue: TThoriumValue): TThoriumType;
      // This parses a term. A term can consist of multiple factors which
      // might be combined using a multiplication.

        function Factor(TargetRegister: TThoriumRegisterID; out IsStatic: Boolean; out StaticValue: TThoriumValue): TThoriumType;
        ...
        begin
          ...
          begin
            ...
            begin
              Result := Factor(TargetRegister, Static, Value);
              case Sym of
                tsNot:
                begin
                  if Static then
          ...
          case CurrentSym of
            ...
            begin
              ...
              Result := Expression(TargetRegister, NeedStatic, @IsStatic, @StaticValue);
              if HasError then
                Exit;
              if not ExpectSymbol([tsCloseBracket]) then
                Exit;
      ...
      begin
        ...
        // Parse the factor
        Result := Factor(TargetRegister, Static1, Value1);
        if HasError then
          Exit;
        ...
        begin
          ...
          OperandType := Factor(RegID2, Static2, Value2);
          if HasError then
            Exit;
          ...
          begin
            if not ThoriumCompareType(OperandType, Result) then
            begin
          ...
          if ThoriumTypeNeedsClear(PrevType) and not ThoriumCompareType(PrevType, Result) then
          begin
            if not GetFreeRegister(trEXP, RegBuf) then
              Exit;
          ...
          begin
            if not ThoriumCompareType(PrevType, Result) then
    ...
    begin
      ...
      Result := Term(TargetRegister, Static1, Value1);
      if HasError then
        Exit;
      // Allocate two more registers
      if not GetFreeRegister(trEXP, RegID2, True) then
      ...
      begin
        ...
        if Static2 then
          ThoriumFreeValue(Value2);
        OperandType := Term(RegID2, Static2, Value2);
        if HasError then
          Exit;
    ...
    function Expression(TargetRegister: TThoriumRegisterID; NeedStatic: Boolean = False; IsStatic: PBoolean = nil; StaticValue: PThoriumValue = nil): TThoriumType; inline;
    ...
    begin
      ...
      Result := SimpleExpression(TargetRegister, NeedStatic, @Static1, @Value1);
      if HasError then
        Exit;
        ...
        begin
          ...
          begin
            ...
            case MyHook2[I].Instruction of
              ...
              tiMOVES:
                TThoriumInstructionMOVES(MyHook2[I]).TRI := CacheRegister;
            else
        ...
        begin
          ...
          begin
            ...
            case MyHook1[I].Instruction of
              ...
              tiMOVES:
                TThoriumInstructionMOVES(MyHook1[I]).TRI := CacheRegister;
            else
              // It was not possible to change it accordingly...
              Static1 := False;
            end;
        ...
        begin
          ...
          case CurrentSym of
            ...
            begin
              ...
              if not NeedTypeOperation(IdentInfo1.FinalType, ExpressionType, toAssign, ExpressionType) then
                Exit;
              // And append the code
    ...
    begin
      ...
      begin
        ...
        Func := TThoriumFunction.Create(Self);
...
begin
  ...
  begin
    FCompiled := True;
    // This is used for dummy modules.
    Exit;
  end;
  ...
  try
    ...
    CodeHook1 := nil;
    CodeHook2 := nil;
    FillByte(RegisterUsage, THORIUM_REGISTER_MASK_SIZE, 0);
...
begin
  ...
  begin
    ...
    begin
      ...
      Func := TThoriumHostFunctionBase(FHostFuncUsage[I]);
      if Func is TThoriumHostMethodBase then
      begin
        if TThoriumHostMethodBase(Func).FExtendedType is TThoriumRTTIObjectType then
          WriteLn(TThoriumRTTIObjectType(TThoriumHostMethodBase(Func).FExtendedType).FBaseClass.ClassName, '.', Func.Name)
        else
          WriteLn(TThoriumHostMethodBase(Func).FExtendedType.ClassName, '.', Func.Name);
      end
      else
      begin
        WriteLn(Func.Name);
...
function TThoriumModule.IsTypeCompatible(Value1, Value2: TThoriumType; Operation: TThoriumOperation; out ResultType: TThoriumType): Boolean;
...
begin
  case Value1.ValueType of
    ...
    vtExtendedType:
    begin
      Value1.Extended.IsTypeCompatible(Value1, Value2, Operation, ResultType);
    end;
    vtFunction: Result := False;
  end;
end;

function TThoriumModule.IsTypeOperationAvailable(Value: TThoriumType; Operation: TThoriumOperation; out ResultType: TThoriumType): Boolean;
...
begin
  case Value.ValueType of
    ...
    vtExtendedType:
    begin
      Value.Extended.IsTypeOperationAvailable(Value, Operation, ResultType);
    end;
    vtFunction: Result := False;
  end;
end;
...
var
  ...
  begin
    ...
    begin
      ...
      _NewEntry^.Value.Extended.Size := TThoriumHostObjectType(ExtendedType).GetNeededMemoryAmount;
      if _NewEntry^.Value.Extended.Size = 0 then
        _NewEntry^.Value.Extended.Value := nil
      else
        GetMem(_NewEntry^.Value.Extended.Value, _NewEntry^.Value.Extended.Size);
  ...
  begin
    ...
    begin
      ...
      FRegisters[TRI].Extended.Size := TThoriumHostObjectType(ExtendedType).GetNeededMemoryAmount;
      if FRegisters[TRI].Extended.Size = 0 then
        FRegisters[TRI].Extended.Value := nil
      else
        GetMem(FRegisters[TRI].Extended.Value, FRegisters[TRI].Extended.Size);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            _Operand1[-(_Operand1^.Params+1)] := _Operand1[1];
            FStack.Pop(1+_Operand1^.Params);
          end;
        end;
      end
      else
...
procedure TThoriumVirtualMachine.Execute(StartModuleIndex: Integer; StartInstruction: Integer; CreateDefaultStackFrame: Boolean = False);
var
  Frame: PThoriumStackEntry;
  {$ifdef DebugToConsole}
  OldIdx: Integer;
...
begin
  ...
  {$ifdef DebugToConsole}
    ...
    WriteLn('CRASH at Instruction ', IntToHex(FCurrentInstructionIdx, 8), ': ', ThoriumInstructionToStr((FCurrentModule.FInstructions.Instruction[FCurrentInstructionIdx])^));
    DumpStack;
    WriteLn;
    raise;
  end;
    ...
    case Entry^._Type of
      ...
      begin
        case Entry^.Value._Type of
          ...
          vtExtendedType: Write('V:', ThoriumTypeName(Entry^.Value));
        end;
...
begin
  ...
  case AInstruction.Instruction of
    ...
    tiMOVES: with TThoriumInstructionMOVES(AInstruction) do Result := Result + Format('%%%s', [ThoriumRegisterToStr(TRI)]);
    ...
    tiCALL: with TThoriumInstructionCALL(AInstruction) do Result := Result + Format('$0x%.8x', [EntryPoint]);
